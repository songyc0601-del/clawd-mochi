...
A physical desk companion inspired by **Clawd** — the pixel-crab mascot of Claude Code by Anthropic. An ESP32-C3 drives a 1.54" color TFT display and hosts a mobile web controller, USB serial controls, Codex progress display, and browser-based OTA updates — no app, internet, or cloud required.
...
Clawd Mochi sits on your desk and shows animated expressions or your current Codex work phase on a small color display. You control it from any phone or browser through its built-in WiFi hotspot or, after setup, through your normal 2.4 GHz LAN:

- **Companion expressions** — normal, focus, and happy eyes
- **Codex work status** — IDLE, PLAN, CODE, TEST, DONE, and BLOCK phases
...
The work-status panel is read-only. Codex or automation scripts push phase updates to the device.

### Codex status display

The firmware accepts these Codex status states:

| Phase | Meaning |
| --- | --- |
| OFFLINE | Codex is offline; return to the default Clawd animation |
| IDLE | Codex is online and ready, with no active task |
...
`OFFLINE` is not a Codex work phase. It is used when Codex is not open, the watcher is not running, or the device-side Codex heartbeat times out. Non-offline states show the Codex core-pulse status screen.
...
On WSL / Linux / macOS, run the watcher:

```bash
./tools/codex-watch.sh --device-url http://device-lan-ip
