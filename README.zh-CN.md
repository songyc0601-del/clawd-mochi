...
- 启动后短暂显示 WiFi 信息，然后进入 `CODEX` + Clawd 眼睛待机页。
- Codex 阶段状态：
  - `IDLE`：待机 Logo
  - `PLAN`：规划中
  - `CODE`：编码中
  ...
  - `DONE`：已完成
  - `BLOCK`：被阻塞
- `PLAN / CODE / TEST` 显示闪烁黄灯和 1/4 至 3/4 进度；`DONE` 显示绿灯；`BLOCK` 显示红灯。
- 电脑也可以通过 USB 串口脚本推送状态。
- 当前 Codex Desktop 已通过 `tools/codex-notify.ps1` 对接：每个工作回合结束时自动推送 `DONE`，并保留原有 Computer Use 通知。
- 项目根目录的 `AGENTS.md` 要求 Codex 在规划、编辑和验证时主动推送 `PLAN / CODE / TEST`。
